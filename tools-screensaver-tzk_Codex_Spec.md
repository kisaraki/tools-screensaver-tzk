...
產品 v0.15.3 將氣象資訊面板本體遮色不透明度由 30% 減半至 15%，背景透光係數改為 0.85；廣域反射強度減半至 0.065。保留既有面板大小、平滑模糊與曲面邊緣折射，見 [Phase 26](docs/phase26-report.md)。

產品 v0.15.2 的氣象資訊面板邊長改為 min(螢幕寬 26%, 背景高 70%)，通常比 v0.15.1 減少約 31% 面積。採連續背景模糊、曲面邊緣位移折射與色散、高光反射、拋光唇邊及柔和投影；文字移除描邊。此修訂覆蓋以下舊版資訊卡高度規則，見 [Phase 25](docs/phase25-report.md)。
...
- v2.9 新增 Phase 22：明確驗證自訂來源可跨程序與重新開機恢復，並將每次來源的隨機起點改由原生工作階段產生後傳入播放器；詳第 24 節。
...
```text
請依 tools-screensaver-tzk_Codex_Spec.md v2.9 實作指定 Phase。
...
## 24. v2.9／Phase 22：來源持久化與隨機起播保證

產品 v0.14.1，schema 維持 9。五場景的自訂來源是目前 Windows 使用者的持久偏好，主設定按「確定」後保存於 HKCU；每次新程序啟動皆重新讀取，因此登出、重新開機或再次啟動後仍可恢復。不得只放在 dialog draft、WebView2 profile 或程序記憶體。不同 Windows 帳號各自保存，解除安裝仍沿用保留個人偏好的既有規則。

每個螢幕的播放工作階段必須有獨立於來源選擇的起點 PRNG。首次來源、排程切換、失敗復原及預抓候選正式啟用時，每次都推進 PRNG 並產生 `181～539` 秒。原生端把該次 `startSeconds` 放入 player load command；本機 shell 僅接受此範圍的整數。直接影片、清單初載、清單隨機選片及預抓切換使用該次來源起點；影片結束重播同片時也重新抽選。隨機值允許偶然重複，不得以固定 180 秒或保存上次位置取代。

測試必須證明：五組來源寫入實際專用 registry test key、關閉 store 後由新 store 完整讀回；每次來源啟用都寫入合法範圍；連續啟用會推進 PRNG；load script 明確含起點；0 或超界輸入在原生 helper 被限制，shell 亦拒絕缺值或超界命令。遠端測試不得啟動正式設定 UI、全螢幕、安裝程式或 UAC。
