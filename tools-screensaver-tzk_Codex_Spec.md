...
- 原稿的需求、四種字型模式、原有兩種畫面及 Phase 0～5 均保留。v2.1 的 Phase 14 新增指定播放清單及兩種旅行模式；v2.2 的 Phase 15 改良列車駕駛室兩側質感，並把散步場景改成不含眼球或血管的全畫面攝影暗角。
...
- 文件版本與軟體版本分開；文件 v2.2 對應旅行場景視覺修正的目標軟體版號為 `0.10.1`。
...
```text
請依 tools-screensaver-tzk_Codex_Spec.md v2.2 實作指定 Phase。
