...
if (-not $OutputDirectory) { $OutputDirectory = Join-Path $phase2ProjectRoot 'docs/evidence/phase23/weather-fixtures' }
$phase2OutputRoot = [System.IO.Path]::GetFullPath($OutputDirectory)
$phase2RawRoot = Join-Path $phase2ProjectRoot ('target/weather-fixtures-' + [Guid]::NewGuid().ToString('N'))
...
try {
    ...
    cargo test --locked --lib export_weather_fixtures -- --ignored --nocapture
