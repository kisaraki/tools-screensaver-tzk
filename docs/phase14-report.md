...
- 每次全螢幕啟動與設定時間到期時，YouTube IFrame Player API 重新讀取播放清單、shuffle 並隨機選片；不保存清單、不使用 API key、不解析 YouTube HTML。
...
`PlaybackHealthy`、實際播放清單內容、地區限制、禁止嵌入影片、眨眼動畫的實際動態觀感、多螢幕播放、長時間 GPU／記憶體、安裝／升級／解除安裝及 Windows 11 均為 `NOT TESTED`。HTTP 200 與靜態 HTML fixture 不代表影片已進入 `PLAYING`。
