...
| 範圍 | 實作結果 |
| --- | --- |
| 快照與協調器 | `FrameSnapshot` 是每個 generation 的不可變輸入；單一 coordinator 每次只取一次本機時間與 `GetTickCount64`，所有 surface 共用同一快照。離機作業番茄鐘模式的倒數／閃爍為 100 ms 更新，標準桌曆暨時鐘模式及閃爍完成後為 1 s。時間變更與電源恢復會重新取樣，不逐 tick 補跑。 |
| 標準桌曆暨時鐘模式 | 完成圓角方形 60 刻度鐘面、連續時／分針角度、秒針、中心軸，以及 Gregorian 月曆、固定週一首欄、六個日期列、年月寬度 fallback 與今日實心圓。 |
...
| 開發入口 | Debug 接受 `--dev-render=time-date` 與 `--dev-render=countdown`，建立可縮放的一般視窗並沿用正常 cleanup；離機作業番茄鐘模式固定從 5 分鐘開始。Release 對兩個旗標皆回傳 code 2。 |
...
| UT13～UT14 | PASS | 本機時間調整只更新標準桌曆暨時鐘模式（`TimeDate`）；離機作業番茄鐘模式的倒數維持 monotonic deadline；跳過 tick／睡眠後直接依 deadline 重算並 clamp 為零。 |
...
[Phase 2 視覺檢查](phase2-visuals.md) 保存 25 張由本程式實際 GDI renderer 產生的無損 PNG，涵蓋標準桌曆暨時鐘模式與離機作業番茄鐘模式、四色、三種目前可直接驅動的字型路徑、800×369、1920×1080、3840×2160、1080×1920、320×180、120×80，以及最後十秒／暗半週期／完成狀態。標準桌曆暨時鐘模式的固定 `TimeDate` fixture 為 2023-12-31 12:15:40，未修改系統時鐘；所有影像的安全矩形外均驗證為純黑。
...
依 Phase 2 要求，同時啟動標準桌曆暨時鐘模式（`TimeDate`）與離機作業番茄鐘模式（`Countdown`）的 Debug 一般視窗，從 0 秒至 600 秒每分鐘取樣。以下以第 60 秒預熱點至第 600 秒計算；CPU 公式為 `100×程序 CPU 秒增量/(牆鐘秒×8 logical processors)`。

| 模式 | 全機 CPU 平均 | Working set | Private bytes | GDI objects | USER objects |
| --- | ---: | ---: | ---: | ---: | ---: |
| 標準桌曆暨時鐘模式（`TimeDate`） | 0.0723% | 10,817,536→10,821,632（+4,096） | 1,851,392→1,818,624（−32,768） | 26→26；範圍 26～27 | 14→14 |
| 離機作業番茄鐘模式（`Countdown`） | 0.0275% | 10,813,440→10,821,632（+8,192） | 1,859,584→1,826,816（−32,768） | 29→30；範圍 29～30 | 14→14 |
...
| Windows 系統閒置／安全桌面的離機作業番茄鐘模式流程 | 正式倒數輸入、模式偏好與 `.scr` 尚未存在，屬 Phase 3 及後續封裝驗收。 |
