...
四種播放清單模式在全螢幕啟動時使用 YouTube 官方 IFrame Player API 的 `loadPlaylist` 讀取當下清單，再呼叫 `setShuffle(true)` 與 `getPlaylist()`，從回傳的影片 ID 中隨機選取並以 `loadVideoById` 播放。這個做法不需要 API key，也不抓取或解析 YouTube 網頁 HTML。清單讀取最多等待 5 秒；player error、清單空白或 20 秒內未開始播放時會回報失敗，原生控制器於 30 秒後有界重試。

每次載入與同片重播都指定 `startSeconds=180`，player 會從最接近約 3:00 的 keyframe 開始；直播、短於三分鐘或來源不支援 seek 時，YouTube 可忽略或調整起點。排程輪換前最後一分鐘，原生 worker 先解析下一個來源，WebView shell 由已讀取的清單預選不同影片並預熱其縮圖/CDN 連線。切換時直接載入該候選；若無有效候選則回退到重新讀取清單。產品不建立第二個隱藏 player，也不在背景播放影音。
...
正式影片固定靜音，設定 `controls=0`、`cc_load_policy=0`、`iv_load_policy=3`、`disablekb=1` 與 `fs=0`，並停用 player 的滑鼠事件，避免顯示控制列、預設字幕、註解與全螢幕按鈕。YouTube 已停用 `showinfo` 與 `modestbranding` 參數，因此平台仍可能短暫顯示必要的標題或品牌資訊，產品不以覆蓋層遮住第三方標示。程式不下載、錄製、轉碼、代理、保存或重新託管影片。場景框、地點與狀態位於 player 外；地方散策的本機眨眼動畫只在過場期間覆蓋 player。系統 preview、設定 preview、日期時鐘與番茄鐘不建立 WebView2，也不連公開網站。
