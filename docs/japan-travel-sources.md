...
自 v0.14.1 起，以實際專用 registry adapter 測試確認五個場景的來源在寫入後關閉並重新開啟仍可完整讀回。正式程式每次啟動皆重新載入目前 Windows 使用者的 HKCU，因此重新登入、開機或再次啟動後會自動恢復；不同使用者帳號互不共用。每次來源正式啟用時，原生工作階段另行產生 181～539 秒並傳給播放器，直接影片、清單選片與預抓切換使用該次值；片尾重播再抽新值。
