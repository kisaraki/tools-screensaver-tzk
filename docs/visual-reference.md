...
![平滑漸層暗色藥劑瓶玻璃番茄鐘面板](evidence/phase19/fixtures/23-Countdown-1920x1080-dpi96-p2-SevenSegment-size.png)

新版移除貫穿整個面板的實色亮帶與暗帶。中央採近黑褐透光核心，上下以連續漸層衰減，左右以琥珀褐漸層模擬厚瓶壁折射；外圈只保留細銅色唇邊，左上加入短局部柔光。這些變更降低實心巧克力分格感，讓材質更接近暗色藥劑瓶玻璃。
...
鐵灰色固定為 `RGB(154,160,163)`，自動模式每 2 分鐘循環七種固定色。Phase 18 的初版番茄鐘面板使用深棕內外層、棕銅框、上方實色反光與底部暗帶；此視覺已由 v0.12.1 取代。Phase 18 共匯出 55 張 GDI fixture，保留作版本證據。

## v0.13.0 旅行框景

![御運轉士窗孔填滿檢查](evidence/phase20/html/train-cab-1600x1000.png)

御運轉士的 16:9 player 在寬螢幕窗孔內置中放大，左右填滿，少量上下內容由窗孔裁切。地方散策保留 0.4% 外緣作為游標停放區；閉眼與睜眼動畫的實際動態觀感仍需可互動 Win10 驗收。
...
以上是程式內附的靜態素材。GDI 預覽以 Windows Imaging Component（WIC）本機解碼，設定與系統預覽不下載圖片、不連網；正式全螢幕的本機 HTML 使用同圖並放置 16:9 YouTube player。御運轉士由窗孔等量裁掉少量上下影像以填滿較寬開口，其餘框景不裁切；地名與狀態不覆蓋影片、品牌、廣告或控制項。Pages 顯示相同素材，沒有載入第三方影片。
