...
規格文件：v1.3（修訂版）<br>
執行日期：2026-09-06<br>
Source revision：本報告隨 Git tag `v0.2.0` 鎖定；Phase 0～5 歷史結果由 tag `v0.1.1` 追溯<br>
...
| 來源 HTTP probe | PASS | 2026-09-06T22:34:09.5322337+08:00；日本目錄正常，8／8 內建候選可解析；產品 WinHTTP／parser 即時測試也通過；[source-health.json](evidence/phase6/source-health.json) |
| 非互動 smoke | PASS | x64 PE、v0.2.0 resources／manifest、第三 radio、網路揭露、imports、靜態 CRT、無 UI 錯誤參數與 registry 不變；[smoke-report.json](evidence/phase6/smoke/smoke-report.json) |
...
| --- | --- | --- | --- | --- |
| AC01 | NOT TESTED | v0.2.0 fmt、Clippy、tests、Release、PE／resources／imports 全部實跑通過 | Phase 6 build、smoke | MT15 的無開發工具乾淨 Win10 尚無環境 |
| AC02 | PASS | Debug／Release 真實命令列與原生視窗測試沿用 Phase 4 證據；v0.2.0 無 UI 錯誤參數重跑通過 | Phase 4 `native-release.txt`、UT01～03、Phase 6 smoke | — |
| AC03 | NOT TESTED | 真實跨程序 parent、三種 host DPI context、resize／退出／刷新已有基線證據 | Phase 3 native、Phase 4 native Release | v0.2.0 未重跑互動 host；缺實體混合 100%／150%／200% 桌面與 Windows 設定頁 |
| AC04 | NOT TESTED | 原生設定、preview、ChooseFont、提交／取消閉環已有 Phase 3 證據；v0.2.0 第三 radio resource 通過 | Phase 3 report／截圖、Phase 6 smoke | 安裝後 Windows 設定頁調度及第三模式互動預覽未測 |
...
| AC10 | PASS | 專用測試 key 與 fake store 驗證型別、schema 3、mode 2 round-trip、取消、rollback、未知值 | UT20～24、UT26 | — |
...
| AC13 | NOT TESTED | v0.2.0 Setup 已編譯且版本／hash 一致 | Phase 6 package、`SHA256SUMS.txt` | 沒有實際安裝、列舉、移除；遠端不觸發 UAC |
| AC14 | NOT TESTED | 非 System32 helper code 4 且四個螢幕保護 registry 值不變 | Phase 6 smoke | 原使用者成功路徑、另一管理員 UAC、直接提權 installer 未測 |
| AC15 | PASS | README、規格、來源紀錄、測試、`.scr`、Setup、hash、Phase 6 與逐項報告均存在 | repository 交付清單 | 成品明列 NotSigned 與未測限制 |
| AC16 | NOT TESTED | 自製 HTML／CSS shell 與 GDI fallback、完整 player rect／外部 caption 的 deterministic test 通過 | UT34、Phase 6 source review | 實際 A380 構圖、player、地點文字及多螢幕未觀察 |
...
| UT26 | PASS | schema 3 的 `JapanTravel=2` round-trip；舊 schema 2 與 future schema 保護 |
...
| UT34 | PASS | GDI 旅行 layout 小尺寸測試、單一 HTML player 及 caption 在 player 外的結構測試 |

Phase 5 的 helper 精確旗標與非 System32 真實 binary 拒絕測試在 v0.2.0 也重跑通過。WebView2 Runtime probe 是 ignored host prerequisite test，已由本次非互動命令明確執行；它不開 UI。

## MT01～MT20

| ID | 狀態 | 已取得的證據 | 缺少條件 |
| --- | --- | --- | --- |
| MT01 | NOT TESTED | Win10 build與歷史 `/s`／`/p`／`/c` 測試 | v0.2.0 安裝、列舉、系統 preview、解除安裝未測 |
...
| MT05 | NOT TESTED | 真實 host 保存後 2 秒刷新、取消不寫的 Phase 3 證據 | Windows 設定頁中的已安裝 v0.2.0 尚未測 |
...
| 成品 | Bytes | SHA-256 | 狀態 |
| --- | ---: | --- | --- |
| `dist/MyDateTimeScreensaver.scr` | 802,816 | `73e971462502b98e06531fac92d356cc01e60095d8f7fe2d34f861c91756b21d` | Build／smoke PASS；NotSigned |
| `dist/MyDateTimeScreensaver-Setup.exe` | 2,319,263 | `ddc3a86d0d9d8aeb2ac353b1cc1a97b1e0bd3f53807bf2cc931fbd89e3935372` | Package／版本檢查 PASS；實際安裝 NOT TESTED；NotSigned |

Phase 5 的 v0.1.1 hash 保留在 Phase 5 報告與該 Release，不再列為目前成品。完成 Windows 10 完整驗收仍需在可互動本機環境補做上述必要項目；遠端工作階段不觸發 UAC。程式碼簽章憑證尚未提供，v0.2.0 成品維持 NotSigned。
