...
Source revision：本報告隨 Git tag `v0.9.0` 鎖定；Phase 0～5 歷史結果由 tag `v0.1.1` 追溯<br>
...
| 自動測試 | PASS | 預設 57 passed：lib 37、CLI 8、native noninteractive 2、layout 10；9 個測試預設 ignored；50 張 GDI 圖片重新匯出 |
| 置中版面／GDI fixture | PASS | 64%×60% 中央內容區、小型 preview、直向、4K、七個 palette 選項與三種旅行場景；[Phase 13 report](phase13-report.md) |
...
| 非互動 smoke | PASS | v0.9.0、ProductName／FileDescription、第三模式、三種旅行場景、七個色彩 radio、來源切換欄位、imports、靜態 CRT、無 UI 錯誤參數與 registry 不變；[smoke-report.json](evidence/phase13/smoke/smoke-report.json) |
...
另完成 6 張離線 headless Edge 靜態 HTML 版面檢查；產品播放器腳本已移除，阻擋 host 解析，只顯示影片位置色塊，沒有建立實際 player。證據見 [Phase 13 report](phase13-report.md)。
...
| --- | --- | --- | --- | --- |
| AC01 | NOT TESTED | v0.9.0 fmt、Clippy、tests、Release、PE／resources／imports 全部實跑通過 | Phase 13 report、smoke | MT15 的無開發工具乾淨 Win10 尚無環境 |
...
| AC04 | NOT TESTED | 原生設定、preview、ChooseFont、提交／取消閉環已有 Phase 3 證據；v0.9.0 三場景及七色 radio resource 通過 | Phase 3 report／截圖、Phase 13 smoke | 安裝後 Windows 設定頁調度及新增選項互動預覽未測 |
...
| AC10 | PASS | 專用測試 key 與 fake store 驗證型別、schema 6、TravelStyle 0／1／2 與 ColorPreset 0～6 round-trip、舊 schema 回退、取消、rollback、未知值 | UT20～24、UT26 | — |
...
| AC13 | NOT TESTED | v0.9.0 Setup 已編譯且版本／hash 一致 | Phase 13 report、`SHA256SUMS.txt` | 沒有實際安裝、列舉、移除或覆蓋升級；遠端不觸發 UAC |
...
| AC16 | NOT TESTED | 三場景 HTML／CSS shell、GDI fallback、完整 player rect／外部 caption 的 deterministic test 及 fixture 通過 | UT26、UT34、UT39、Phase 13 report | 三種場景的實際 player、地點文字及多螢幕未觀察 |
...
| UT34 | PASS | 三場景 GDI 小尺寸／800×450 fixture、每種 HTML shell 單一 player、正確場景 class／aria label 及 caption 位於 player 外 |
...
| UT39 | PASS | 三張 1586×992 PNG 解碼成 32-bpp、快取重用、無效 PNG 拒絕、既有 MTA 生命週期保留與離屏 GDI 繪製 |
...
| 成品 | Bytes | SHA-256 | 狀態 |
| --- | ---: | --- | --- |
| `dist/tools-screensaver-tzk.scr` | 7,099,904 | `09f7e63fbcb8b46b58016b8009e023f7b5d26de8074c3b02d57138a6aaaca8aa` | Build／smoke PASS；NotSigned |
| `dist/tools-screensaver-tzk-Setup.exe` | 10,208,748 | `fe83b4c0135247107111c9aab8bb3c18afe1082ad2dbe7be49f0104dfe8b1176` | Package／版本檢查 PASS；實際安裝 NOT TESTED；NotSigned |

Phase 5 的 v0.1.1 hash 保留在 Phase 5 報告與該 Release，不再列為目前成品。完成 Windows 10 完整驗收仍需在可互動本機環境補做上述必要項目；遠端工作階段不觸發 UAC。程式碼簽章憑證尚未提供，v0.9.0 成品維持 NotSigned。
