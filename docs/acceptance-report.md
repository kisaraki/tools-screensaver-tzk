...
Source revision：本報告隨 Git tag `v0.8.0` 鎖定；Phase 0～5 歷史結果由 tag `v0.1.1` 追溯<br>
...
| 自動測試 | PASS | 預設 56 passed：lib 36、CLI 8、native noninteractive 2、layout 10；9 個測試預設 ignored；40 張 GDI 圖片重新匯出 |
| 置中版面／GDI fixture | PASS | 64%×60% 中央內容區、小型 preview、直向、4K 與旅行場景；40 張圖片重新匯出，包含縮小鐘面數字與擬真旅行圖像；[Phase 12 report](phase12-report.md) |
...
| 非互動 smoke | PASS | v0.8.0、ProductName／FileDescription、第三模式、兩種旅行場景 radio、來源切換欄位、imports、靜態 CRT、無 UI 錯誤參數與 registry 不變；[smoke-report.json](evidence/phase12/smoke/smoke-report.json) |
...
另完成 4 張離線 headless Edge 靜態 HTML 版面檢查；產品播放器腳本已移除，阻擋 host 解析，只顯示影片位置色塊，沒有建立實際 player。證據見 [Phase 12 report](phase12-report.md)。
...
| --- | --- | --- | --- | --- |
| AC01 | NOT TESTED | v0.8.0 fmt、Clippy、tests、Release、PE／resources／imports 全部實跑通過 | Phase 12 report、smoke | MT15 的無開發工具乾淨 Win10 尚無環境 |
| AC02 | PASS | Debug／Release 真實命令列與原生視窗測試沿用 Phase 4 證據；v0.8.0 無 UI 錯誤參數重跑通過 | Phase 4 `native-release.txt`、UT01～03、Phase 12 smoke | — |
| AC03 | NOT TESTED | 真實跨程序 parent、三種 host DPI context、resize／退出／刷新已有基線證據 | Phase 3 native、Phase 4 native Release | v0.8.0 未重跑互動 host；缺實體混合 100%／150%／200% 桌面與 Windows 設定頁 |
| AC04 | NOT TESTED | 原生設定、preview、ChooseFont、提交／取消閉環已有 Phase 3 證據；v0.8.0 第三模式及雙場景 radio resource 通過 | Phase 3 report／截圖、Phase 12 smoke | 安裝後 Windows 設定頁調度及雙場景互動預覽未測 |
...
| AC10 | PASS | 專用測試 key 與 fake store 驗證型別、schema 5、mode 2 與 TravelStyle 0／1 round-trip、schema 3 回退、取消、rollback、未知值 | UT20～24、UT26 | — |
...
| AC13 | NOT TESTED | v0.8.0 Setup 已編譯且版本／hash 一致 | Phase 12 report、`SHA256SUMS.txt` | 沒有實際安裝、列舉、移除或改名前版本覆蓋升級；遠端不觸發 UAC |
| AC14 | NOT TESTED | 非 System32 helper code 4 且四個螢幕保護 registry 值不變 | Phase 12 smoke | 原使用者成功路徑、另一管理員 UAC、直接提權 installer 未測 |
| AC15 | PASS | README、規格、來源紀錄、測試、`.scr`、Setup、hash、Phase 6～12 與逐項報告均存在 | repository 交付清單 | 成品明列 NotSigned 與未測限制 |
...
| UT26 | PASS | schema 3 的 `JapanTravel=2` 相容並預設自在飛行；schema 5 的 `TravelStyle=0/1` round-trip；future schema 保護 |
...
| UT39 | PASS | 兩張 1586×992 PNG 解碼成 32-bpp、快取重用、無效 PNG 拒絕、既有 MTA 生命週期保留與離屏 GDI 繪製 |

Phase 5 的 helper 精確旗標與非 System32 真實 binary 拒絕測試在 v0.8.0 也重跑通過。來源 probe 沿用 Phase 8；本次 Runtime 只以 Setup 共用函式讀取 registry，沒有建立 WebView2 controller 或執行 Runtime installer。

## MT01～MT20

| ID | 狀態 | 已取得的證據 | 缺少條件 |
| --- | --- | --- | --- |
| MT01 | NOT TESTED | Win10 build與歷史 `/s`／`/p`／`/c` 測試 | v0.8.0 安裝、列舉、系統 preview、解除安裝未測 |
...
| MT05 | NOT TESTED | 真實 host 保存後 2 秒刷新、取消不寫的 Phase 3 證據 | Windows 設定頁中的已安裝 v0.8.0 尚未測 |
...
| 成品 | Bytes | SHA-256 | 狀態 |
| --- | ---: | --- | --- |
| `dist/tools-screensaver-tzk.scr` | 4,484,096 | `827e0c9b45e4820cb00dc337cac7321bdcb0a3e06ff06f1c2e35c3028f45b281` | Build／smoke PASS；NotSigned |
| `dist/tools-screensaver-tzk-Setup.exe` | 7,619,663 | `f92160cb1334db2a707014181916d82810cd876ee45b5d6ee505b334741a3e95` | Package／版本檢查 PASS；實際安裝 NOT TESTED；NotSigned |

Phase 5 的 v0.1.1 hash 保留在 Phase 5 報告與該 Release，不再列為目前成品。完成 Windows 10 完整驗收仍需在可互動本機環境補做上述必要項目；遠端工作階段不觸發 UAC。程式碼簽章憑證尚未提供，v0.8.0 成品維持 NotSigned。
