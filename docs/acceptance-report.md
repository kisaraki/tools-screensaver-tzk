...
軟體版本：0.10.0 開發候選版<br>
規格文件：v2.1（修訂版）<br>
執行日期：2026-09-07～2026-09-08（建置與續作驗證）<br>
Source revision：本報告隨 Git tag `v0.10.0` 鎖定；Phase 0～5 歷史結果由 tag `v0.1.1` 追溯<br>
...
| 自動測試 | PASS | 預設 58 passed：lib 38、CLI 8、native noninteractive 2、layout 10；9 個測試預設 ignored；52 張 GDI 圖片重新匯出 |
| 置中版面／旅行 fixture | PASS | 64%×60% 離線模式中央內容區、五種旅行場景；散步視野 60%×54% 並有眨眼結構；[Phase 14 report](phase14-report.md) |
...
| 來源 HTTP probe | PASS | 2026-09-08：tw.live 目錄正常、8／8 候選可解析，4／4 指定 playlist embed endpoint 回傳 HTTP 200；[source-health.json](evidence/phase14/source-health.json) |
| 非互動 smoke | PASS | v0.10.0、五種旅行場景、七個色彩 radio、來源切換欄位、imports、靜態 CRT、無 UI 錯誤參數與 registry 不變；[smoke-report.json](evidence/phase14/smoke/smoke-report.json) |
...
另完成 10 張離線 headless Edge 靜態 HTML 版面檢查；產品播放器腳本已移除，阻擋 host 解析，只顯示影片位置色塊，沒有建立實際 player。證據見 [Phase 14 report](phase14-report.md)。
...
| --- | --- | --- | --- | --- |
| AC01 | NOT TESTED | v0.10.0 fmt、Clippy、tests、Release、PE／resources／imports 全部實跑通過 | Phase 14 report、smoke | MT15 的無開發工具乾淨 Win10 尚無環境 |
...
| AC04 | NOT TESTED | 原生設定、preview、ChooseFont、提交／取消閉環已有 Phase 3 證據；v0.10.0 五場景及七色 radio resource 通過 | Phase 3 report／截圖、Phase 14 smoke | 安裝後 Windows 設定頁調度及新增選項互動預覽未測 |
...
| AC09 | NOT TESTED | 六種固定色、自動色彩、四字型、fallback 與極端點數有測試／fixture | UT18～22、UT40、visual-reference | 200% 實體 dialog、長時間自動換色與混合 DPI 未測 |
| AC10 | PASS | 專用測試 key 與 fake store 驗證型別、schema 7、TravelStyle 0～4 與 ColorPreset 0～6 round-trip、舊 schema 回退、取消、rollback、未知值 | UT20～24、UT26 | — |
...
| AC13 | NOT TESTED | v0.10.0 Setup 已編譯且版本／hash 一致 | Phase 14 report、`SHA256SUMS.txt` | 沒有實際安裝、列舉、移除或覆蓋升級；遠端不觸發 UAC |
| AC14 | NOT TESTED | 非 System32 helper code 4 且四個螢幕保護 registry 值不變 | Phase 13 smoke | 原使用者成功路徑、另一管理員 UAC、直接提權 installer 未測 |
| AC15 | PASS | README、規格、來源紀錄、測試、`.scr`、Setup、hash、Phase 6～13 與逐項報告均存在 | repository 交付清單 | 成品明列 NotSigned 與未測限制 |
| AC16 | NOT TESTED | 五場景 HTML／CSS shell、GDI fallback、player rect／外部 caption 的 deterministic test 及 fixture 通過 | UT26、UT34、UT39、Phase 14 report | 五種場景的實際 player、地點文字及多螢幕未觀察 |
...
| AC26 | NOT TESTED | 日式旅館原創 PNG 內附，WIC、GDI 與 HTML 皆使用同圖；50 張 GDI 與 6 張 HTML 幾何 fixture 通過 | Phase 13 report、travel-artwork、geometry.json | 實際 YouTube player 與旅館窗框完整性尚未觀察 |
| AC27 | NOT TESTED | schema 6 新色 round-trip；暗淺藍／琥珀色 fixture 與自動色彩 120 秒邊界、完整循環測試通過 | UT26、UT40、Phase 13 fixtures／smoke | 正式設定視窗與多螢幕長時間自動換色未測 |
...
| AC29 | NOT TESTED | 列車駕駛前方與散步原創 PNG、WIC／GDI／HTML fixture 通過；散步中央 60%×54%、700 ms 眨眼與 300 ms 換片點由測試固定 | UT34、UT39、UT41、Phase 14 fixtures | 實際影片中的眨眼過場、多螢幕與 GPU 表現未觀察 |
...
| UT26 | PASS | schema 3 的 `JapanTravel=2` 相容；schema 6 的 `TravelStyle=0/1/2` 與 `ColorPreset=0～6` round-trip；舊 schema 不誤解新版值，future schema 保護 |
...
| UT34 | PASS | 五場景 GDI 小尺寸／800×450 fixture、每種 HTML shell 單一 player、正確場景 class／aria label 及 caption 位於 player 外 |
...
| UT40 | PASS | 自動色彩 119,999／120,000 ms 邊界、六色循環及 720,000 ms 回到第一色 |
...
| 成品 | Bytes | SHA-256 | 狀態 |
| --- | ---: | --- | --- |
| `dist/tools-screensaver-tzk.scr` | 9,368,064 | `b9ccd78fa5ee03df41fe652294be626d5b45910ad32efd938fde65a476a93de7` | Build／smoke PASS；NotSigned |
| `dist/tools-screensaver-tzk-Setup.exe` | 12,431,415 | `51e62b34f345dd74b7ff9e4375dc217a64e6131a75e0c8df59cbfdef4080ddcf` | Package／版本檢查 PASS；實際安裝 NOT TESTED；NotSigned |

Phase 5 的 v0.1.1 hash 保留在 Phase 5 報告與該 Release，不再列為目前成品。完成 Windows 10 完整驗收仍需在可互動本機環境補做上述必要項目；遠端工作階段不觸發 UAC。程式碼簽章憑證尚未提供，v0.10.0 成品維持 NotSigned。
