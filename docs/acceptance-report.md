...
軟體版本：0.15.4 開發候選版<br>
規格文件：v2.10（修訂版）<br>
執行日期：2026-09-07～2026-09-16（建置與續作驗證）<br>
Source revision：目前結果隨 Git tag `v0.15.4` 鎖定；舊結果按各 Phase tag 追溯<br>
...
v0.15.4 增量：氣象玻璃本體透光係數提高至 98%，降低柔化、廣域反射與陰影；保留曲面折射、色散及拋光邊。MIT 授權全文新增 Pages 匿名下載，詳 [Phase 27](phase27-report.md)。

v0.15.3 增量：玻璃本體遮色不透明度由 30% 降至 15%，大面積反射強度減半；保留平滑背景柔化、面板大小與邊緣光學效果，詳 [Phase 26](phase26-report.md)。

v0.15.2 增量：氣象資訊面板通常再縮小約 31% 面積，改為連續模糊、雙線性取樣及曲面邊緣折射、色散與反射的擬真玻璃；文字移除描邊。79 預設 tests、34 安裝政策 checks、24 張 GDI fixtures 與非互動 smoke 通過，詳 [Phase 25](phase25-report.md)。
...
| 目前成品非互動 smoke | PASS | 0.15.4、四主模式、五 dialog、氣象及更新控制項、PE／imports、錯誤 CLI；正式螢幕保護 registry 前後一致；[smoke](evidence/phase27/smoke/smoke-report.json) |
...
| 成品 | Bytes | SHA-256 | 狀態 |
| --- | ---: | --- | --- |
| `dist/tools-screensaver-tzk.scr` | 20,517,888 | `8df12cb9aa35df5bb2fd90a4ca4a2391b8c2ad1ef2cf524536fbad4c667fa023` | Build／smoke PASS；NotSigned |
| `dist/tools-screensaver-tzk-Setup.exe` | 23,314,600 | `ca03095284eb3c4b606f6ee5942e1ee5af8abf73037fb0756cbf373f77b005ad` | Package PASS；實際安裝／升級 NOT TESTED；NotSigned |

舊版 hash 保留在各 Phase 報告與 Release，不再列為目前成品。完成 Windows 10 完整驗收仍需在可互動本機環境補做上述必要項目；遠端工作階段不觸發 UAC。程式碼簽章憑證尚未提供，v0.15.4 成品維持 NotSigned。
